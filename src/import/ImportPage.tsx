...
export function ImportPage() {
  ...
  const parseFiles = async (selected: FileList) => {
    ...
    setEntries([]); setProgress(0); setStatus(`Đang đọc 0/${files.length} file…`);
    const output: ParsedFile[] = [];
    try {
      // Sequential parsing avoids opening many large XLSX workbooks in memory at once.
  ...
  const changeType = (next: DatasetType) => { workerRef.current?.terminate(); setType(next); setEntries([]); setStatus(''); setProgress(0); };
  ...
  return <section><header className="page-heading"><div><p>DATA PIPELINE</p><h1>Nhập dữ liệu</h1></div></header>
    ...
    </article><article className="panel"><h2>2. Kiểm tra và xác nhận</h2>{preview ? <>
    ...
    </> : <p>Chọn file để xem preview. Dữ liệu chưa được gửi lên server ở bước này.</p>}</article></div>
  </section>;
}
