...
export function DateFilter({ value, onChange }: { value: DashboardFilters; onChange: (next: DashboardFilters) => void }) {
  ...
  const month = (offset: number) => {
    ...
    const to = offset === 0 ? now : new Date(now.getFullYear(), now.getMonth(), 0);
    choose({ from: iso(from), to: iso(to) });
  };
  ...
  return <div className="date-picker" ref={ref}>
    ...
    {open && <div className="date-popover">
      <button className={active(yesterday())} onClick={() => choose(yesterday())}>Hôm qua</button>
      <button className={active(preset(7))} onClick={() => choose(preset(7))}>7 ngày qua</button>
      <button className={active(preset(30))} onClick={() => choose(preset(30))}>30 ngày qua</button>
      <button onClick={() => month(0)}>Tháng này</button>
      <button onClick={() => month(-1)}>Tháng trước</button>
      <button className="custom-toggle" onClick={() => setCustom(!custom)}>Tùy chỉnh</button>
      {custom && <div className="custom-range">
        <label>Từ ngày<input type="date" value={value.from} max={value.to} onChange={(e) => onChange({ ...value, from: e.target.value })} /></label>
        <label>Đến ngày<input type="date" value={value.to} min={value.from} onChange={(e) => onChange({ ...value, to: e.target.value })} /></label>
