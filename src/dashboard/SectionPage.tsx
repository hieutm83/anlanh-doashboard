...
export function SectionPage({ section, title, canEdit=false }: { section:string; title:string; canEdit?:boolean }) {
  ...
  return <><header className="page-heading"><h1>{title}</h1><div className="heading-tools">{canEdit&&<OperationsEditor section={section}/>}<button className="export-button" onClick={()=>exportJson(`${section}-${filters.from}-${filters.to}`,query.data)}>JSON</button><button className="export-button" onClick={()=>exportMarkdown(`${section}-${filters.from}-${filters.to}`,rows)}>MD</button><button className="refresh" onClick={()=>query.refetch()}>↻</button><DateFilter value={filters} onChange={setFilters}/></div></header>
    ...
    {!query.isLoading&&!query.error&&<>
      {spec.kpis.length>0&&<section className={`legacy-kpis ${spec.kpis.length>8?'dense':''}`}>{spec.kpis.map((item,index)=>{const value=kpiValue(rows,summary,item.metric,item.ratio);return <article className="legacy-kpi" key={`${item.label}-${index}`}><span>{item.label}</span><strong>{item.currency?`${money.format(value)} ₫`:item.ratio==='ctr'?`${number.format(value)}%`:number.format(value)}</strong><small>{rows.length?`Từ ${rows.length} nhóm dữ liệu`:'Chưa có dữ liệu'}</small></article>})}</section>}
      {spec.charts.length>0&&<section className="legacy-chart-grid">{spec.charts.map((item,index)=><article className="legacy-chart-card" key={`${item.title}-${index}`}><h2>{item.title}</h2><div className="legacy-chart-wrap">{item.title.includes('Bản đồ')?<VietnamGeoMap rows={rows}/>:<DashboardChart config={charts[index]}/>}</div></article>)}</section>}
      {spec.tables.map((table,index)=><article className="legacy-table-card" key={`${table.title}-${index}`}><h2>{table.title}</h2><div className="legacy-table-scroll"><table><thead><tr>{table.columns.map(column=><th key={column}>{columnLabels[column]??column}</th>)}<th>Chi tiết</th></tr></thead><tbody>{rows.length?rows.slice(0,100).map((row,rowIndex)=><tr key={rowIndex}>{table.columns.map(column=><td key={column}>{currencyColumns.has(column)?`${money.format(Number(row[column]??0))} ₫`:typeof row[column]==='number'?number.format(Number(row[column])):String(row[column]??'—')}</td>)}<td><button className="detail-button" onClick={()=>setDetail(row)}>Xem</button></td></tr>):<tr><td colSpan={table.columns.length+1} className="empty-cell">Không có dữ liệu trong khoảng thời gian đã chọn</td></tr>}</tbody></table></div></article>)}
    </>}
