...
export function OverviewPage() {
  ...
  const charts = useMemo(() => {
    ...
    const breakdown = query.data?.costBreakdown?.filter((x) => Number(x.value) > 0) ?? [];
    const provinces = !breakdown.length && query.data?.provinces?.length ? query.data.provinces : [{ name: 'Chưa có dữ liệu', revenue: 1 }];
    ...
    const gmvCost: ChartConfiguration<'doughnut'> = { type: 'doughnut', data: { labels: breakdown.map((x) => x.label), datasets: [{ data: breakdown.map((x) => x.value), backgroundColor: ['#347ff0','#8150ee','#ff963c','#ff5b7d','#22c55e'], borderColor: '#fff', borderWidth: 2 }] }, options: { ...common, cutout: '55%' } };
    return { revenueOrders, aov, customers, province, gmvCost: breakdown.length ? gmvCost : province };
  }, [query.data]);
  ...
  return <>
    ...
    {query.data && <><section className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4"><KpiCard label="TỔNG GMV" kpi={query.data.revenue} currency /><KpiCard label="TỔNG ĐƠN HÀNG" kpi={query.data.orders} /><KpiCard label="AOV" kpi={{ value: query.data.orders.value ? query.data.revenue.value / query.data.orders.value : 0, previous: query.data.orders.previous ? query.data.revenue.previous / query.data.orders.previous : 0, changePct: null }} currency /><KpiCard label="TỈ LỆ KHÁCH MUA LẠI" kpi={query.data.repeatRate} /></section>
      <section className="mb-5 grid grid-cols-1 gap-4 lg:grid-cols-2">{[
        ['Số lượng đơn, đơn hủy và GMV', charts.revenueOrders],
        ['AOV theo thời gian', charts.aov],
        ['Khách cũ và khách mới', charts.customers],
        [query.data.costBreakdown?.length ? 'Cơ cấu GMV tổng' : 'Doanh thu theo Tỉnh/Thành phố (Top 10)', charts.gmvCost],
      ].map(([title,config])=><article className="card" key={String(title)}><h2 className="mb-4 text-center text-xs font-bold text-slate-700">{String(title)}</h2><DashboardChart config={config as ChartConfiguration} /></article>)}</section>
