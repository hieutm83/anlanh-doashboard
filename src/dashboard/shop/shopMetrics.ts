...
export type DailySourceMetrics = { date:string; sources:SourceMetrics[]; total:MetricTotals };
...
const ctorAliases=['ctor (don hang sku)','ctor','ty le them vao gio'];
const cancelledAliases={gmv:['doanh thu bi huy','gmv bi huy'],orders:['so don hang bi huy','don hang bi huy'],sales:['so luong bi huy','so mon bi huy']} as const;
const sampleOrderAliases=['don hang mau','so don hang mau'];
...
function read(payload:Record<string,unknown>,groups:string[],fields:string[]){const es=entries(payload),gs=groups.map(normalize),fs=fields.map(normalize);const grouped=es.find(e=>gs.some(g=>e.group===g||e.group.includes(g))&&fs.some(f=>e.field===f));const fallback=es.find(e=>!e.group&&fs.some(f=>e.field===f));return parseMetric((grouped??fallback)?.value);}
...
export function readSourceMetric(product:RawProduct,key:SourceKey,metric:keyof typeof aliases):number{const def=sourceDefs.find(x=>x.key===key)!;const value=read(product.rawPayload,def.groups,aliases[metric]);if(metric==='gmv')return Math.max(0,value-read(product.rawPayload,def.groups,[...cancelledAliases.gmv]));if(metric==='orders')return Math.max(0,value-read(product.rawPayload,def.groups,[...cancelledAliases.orders])-read(product.rawPayload,def.groups,sampleOrderAliases));if(metric==='sales')return Math.max(0,value-read(product.rawPayload,def.groups,[...cancelledAliases.sales])-read(product.rawPayload,def.groups,sampleOrderAliases));if(metric!=='atc'||value>0)return value;const rawCtor=read(product.rawPayload,def.groups,ctorAliases);const ctor=rawCtor>1?rawCtor/100:rawCtor;return readSourceMetric(product,key,'clicks')*ctor;}
...
export function aggregateSourcesByDate(products:RawProduct[]):DailySourceMetrics[]{const dates=new Map<string,RawProduct[]>();for(const product of products)dates.set(product.date,[...(dates.get(product.date)??[]),product]);return [...dates.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([date,items])=>{const sources=aggregateSources(items);const visible=sources.filter(source=>source.key!=='shopTab');const total=empty();for(const source of visible)(Object.keys(aliases) as Array<keyof typeof aliases>).forEach(metric=>total[metric]+=source[metric]);return {date,sources,total:derive(total)};});}
