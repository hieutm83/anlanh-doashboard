...
export function readSourceMetric(product:RawProduct,key:SourceKey,metric:keyof typeof aliases):number{const def=sourceDefs.find(x=>x.key===key)!;const value=read(product.rawPayload,def.groups,aliases[metric]);if(metric!=='atc'||value>0)return value;const rawCtor=read(product.rawPayload,def.groups,ctorAliases);const ctor=rawCtor>1?rawCtor/100:rawCtor;return readSourceMetric(product,key,'clicks')*ctor;}
