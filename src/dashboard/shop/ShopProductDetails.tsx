...
import { ShopProductSourcesModal } from './ShopDetailModals';
...
export function ShopProductDetails() {
  ...
  </ShopState>{detail&&<ShopProductSourcesModal productId={String(detail.productId??detail.label??'')} productLabel={String(detail.label??'Sản phẩm')} products={state.rawProducts} onClose={()=>setDetail(null)}/>}</>;
}
