...
import { EmptyRow, formatMoney, formatNumber, ShopHeader, ShopState, type ShopRow, useShopChart, useShopSection } from './shared';

export function ShopProductDetails() {
  const state = useShopSection('products'); const { rows } = state; const [detail, setDetail] = useState<ShopRow|null>(null);
  ...
  const ctr = useShopChart(rows, 'CTR', 'clicks', 'line'); const cr = useShopChart(rows, 'CR', 'orders', 'line');
