...
describe('shop traffic metrics',()=>{
  ...
  it('calculates net GMV, orders and sales after cancellations and samples',()=>{const net:RawProduct={...product,rawPayload:{'Buổi LIVE của người bán > GMV đã ghi nhận':'1.000.000','Buổi LIVE của người bán > Doanh thu bị hủy':'100.000','Buổi LIVE của người bán > Đơn hàng đã ghi nhận':'20','Buổi LIVE của người bán > Số đơn hàng bị hủy':'3','Buổi LIVE của người bán > Đơn hàng mẫu':'2','Buổi LIVE của người bán > Số món bán ra đã ghi nhận':'25','Buổi LIVE của người bán > Số lượng bị hủy':'4'}};const live=aggregateSources([net]).find(x=>x.key==='sellerLive')!;expect(live.gmv).toBe(900000);expect(live.orders).toBe(15);expect(live.sales).toBe(19);expect(live.aov).toBe(60000);});
});
